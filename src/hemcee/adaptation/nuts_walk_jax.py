...
from hemcee.kernels.nuts_walk_helper import DAState, da_cond_update
...
class AffineInvariantEnsembleNUTSSampler:
    ...
    def __init__(self, 
        ...
        H_bar_init = jnp.array(0.0)
        log_epsilon_bar_init = jnp.array(0.0)
        self.da_state = DAState(jnp.array(self.step_size), H_bar_init, log_epsilon_bar_init)
    ...
    def ensemble_leapfrog(self,
        ...
        """
        Ensemble leapfrog step with jax.lax.cond control flow.
        """
        n_complement = complement_ensemble.shape[0]

        # Precompute centered complement once (shared by both branches)
        complement_mean = jnp.mean(complement_ensemble, axis=0)
        centered_complement = (complement_ensemble - complement_mean) / jnp.sqrt(n_complement)

        def forward_branch(op):
            theta, r, centered_complement, epsilon = op
            grad = grad_fn(theta)
            r_half = r + 0.5 * epsilon * beta * (grad @ centered_complement.T)
            theta_new = theta + epsilon * beta * (r_half @ centered_complement)
            grad_new = grad_fn(theta_new)
            r_new = r_half + 0.5 * epsilon * beta * (grad_new @ centered_complement.T)
            return theta_new, r_new

        def backward_branch(op):
            theta, r, centered_complement, epsilon = op
            grad = grad_fn(theta)
            r_half = r - 0.5 * epsilon * beta * (grad @ centered_complement.T)
            theta_new = theta - epsilon * beta * (r_half @ centered_complement)
        ...
        # Ensure predicate is a JAX boolean scalar
        pred = (jnp.asarray(direction) == 1)

        theta_new, r_new = jax.lax.cond(
            pred,
            forward_branch,
            backward_branch,
            operand=(theta, r, centered_complement, epsilon)
        )
    ...
    def build_tree(self, theta: jnp.ndarray, r: jnp.ndarray, u: jnp.ndarray,
        ...
        if depth == 0:
            ...
            theta_prime, r_prime = self.ensemble_leapfrog(
                self.grad_log_prob_fn, theta, r, epsilon, self.beta, complement_ensemble, direction)
    ...
    def sample(self, theta_init: jnp.ndarray, num_samples: int,
        ...
        for i in tqdm(range(total_samples)):
            ...
            if adapt_step_size:
                ...
                da_cond_update(i, warmup, mean_accept, target_accept=self.target_accept, t0=self.t0, mu=self.mu, gamma=self.gamma, kappa=self.kappa, state=self.da_state)
...
def test_ensemble_nuts():
    ...
    for total_chains in chain_counts:
        ...
        samples, diagnostics = sampler.sample(
            initial, num_samples=100, warmup=100, 
